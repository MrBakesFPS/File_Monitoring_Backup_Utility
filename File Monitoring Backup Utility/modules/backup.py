...
def create_dest(src_path: Path) -> Path:
    ...
    '''
    path_ext = 1
    dest_dir = dest_path = src_path.parent / Path(f'{src_path.stem}_backups')
    if not dest_dir.exists():
        dest_dir.mkdir()
        
    dest_path = dest_dir / Path(f'{src_path.stem}_backup_{todays_date()}{src_path.suffix}')

    while dest_path.exists():
        path_ext = path_ext + 1
        dest_path = src_path.parent / Path(f'{src_path.stem}_backups') / Path(f'{src_path.stem} backup_{todays_date()}_{path_ext}{src_path.suffix}')
